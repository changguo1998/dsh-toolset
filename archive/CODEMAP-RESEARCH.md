...
> **已归档**：本文是 code-map 插件的前期调研快照（外部链接与结论为调研时状态）。设计结论见 `code-map/docs/DESIGN.md`。

> 用途：为 `code-map/docs/DESIGN.md`（backlog #21-#22 → 结构索引/调用图/项目报告）提供设计依据；外部链接为公开资料，仅作参考。
> 结论速览：同类项目可分 5 流派、3 个决策维度；本项目采用「结构层（ast-grep）+ 语义层（tool-lsp）按需」的轻量混合，不做全量语义索引。
> 取舍结论已落入 `code-map/docs/DESIGN.md` §2，本文保留调研细节与出处。
