...
> 用途：作为 agent 系统设计与实现的架构参考（姊妹文档：`archive/PI-DSH-FEATURE-COMPARISON.md`，已归档的迁移调研；已落地的插件与状态见 `STATUS.md`）。
...
> 落地：已实现为 `knowledge-base` 插件；表结构与接口细节以 `knowledge-base/docs/DESIGN.md` 与源码为准。
...
- 复用宿主既有能力作为实现底座：storage-sqlite、session-query-sqlite（FTS5 基础），契约基线 `dsh-v0.1.7-rc.2`。
...
> 接口对照基于 dsh `0.1.2-rc.1` 源码核对；当前契约基线为 `dsh-v0.1.7-rc.2`（见 `docs/host/DSH-CTX-API.md`）。本节清单已落地为 `task-engine` 插件（首版与边界见 `task-engine/README.md`、`STATUS.md`）。
