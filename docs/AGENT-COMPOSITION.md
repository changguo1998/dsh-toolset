...
- 因此目录式 preset 配置（`$DSH_HOME/.agent-presets/<id>/` 与 `settings.yaml` 的 `agent-presets.default`）在本 profile 下是空配置：该 settings 命名空间没有 provider。TUI 的 `/preset` 提示「agent 预设服务不可用」是 fail-safe 正常路径（装配证据见 `../TUI/DESIGN.md`）。
...
- 配置面同理：0.1.7 起 settings 的命名空间 = **profile 插件条目 id**，写入落 profile 的 `cordis.patch.yml`（`$DSH_HOME/settings.yaml` 只作一次性导入，随后改名 `.imported`）；provider / 凭据 / 插件配置都按「条目 id」定位，细节见 `HOST-UPGRADE-0.1.7-rc.2.md` §3.6。
