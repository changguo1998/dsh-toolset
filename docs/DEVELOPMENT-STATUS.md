...
- TUI 侧现状见 `TUI/docs/STATUS.md`（完成情况、命令面、宿主升级适配；含 herdr pane 外部问题的指向），待办与开放项见 `TUI/docs/BACKLOG.md`——本文件不复述 TUI 细节。
...
- 宿主基线：本地安装 dsh **`0.1.7-rc.2`**（2026-09-25 由 0.1.5-rc.3 升级，随包分发包 283 个；npm `latest` 仍停在 0.1.5-rc.3、候选在 `next`）。官方包清单与分类见 `docs/host/HOST-PACKAGES.md`；该基线内**无 worktree 隔离包、无跨会话消息包**（对应待办 #15 / #30）。
- 0.1.7-rc.2 升级（2026-09-25）：代码侧三处破坏已修并**双栈兼容**——output-compress 接 `ptcRuntime`（`resolve` + `run`），TUI 两处（`jobs` 契约、`/agents` 数据源）见 `TUI/docs/STATUS.md`；`scripts/install.sh` 默认版本与 TUI 文档已同步；宿主与 profile 均已升级（profile 的 `dsh-session-title-all-prompts-llm` → 0.1.7-rc.2），`--dump-config` 含 13 个 `@dsh-toolset/*`，pty 冒烟无 `did not activate`，settings 已迁移（`settings.yaml` → `.imported`）。验收与执行记录见 `docs/host/HOST-UPGRADE-0.1.7-rc.2.md`（§0.1 实施状态）。
