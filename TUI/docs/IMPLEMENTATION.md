...
> 配套：`README.md`（使用与配置）、`TUI/docs/DESIGN.md`（架构/设计）、`SPEC.md`（规格）、`COMMANDS.md` / `COMMANDS-SPEC.md`（命令面）。渲染管线重构（主线 A/B）的实施过程已归档至仓库根 `archive/TUI-RENDER-REFACTOR-RECORD.md`。
