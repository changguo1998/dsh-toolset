...
证据索引：3.1.3 见「实现记录 · 步骤 1」；3.1.2 见「实现记录 · 步骤 2」（含 `/model` 补漏）；3.1.1 见「实现记录 · 步骤 3」。
...
- 真机验证（用户执行，`fffdsh` / `dsh --profile fff`）：
  - **3.1.2 通过**：用户发现 `/model` 面板内仍残留英文键位帮助行（见本文「补漏」），修复后复验通过。
  - **3.1.1 通过**：加临时钩子（审批请求发出 5s 后自动弹 notice，仅用于验证；测完已删除，`git diff` 无残留）——审批面板打开期间该 notice **立即出现在底部输入区**。同时确认语义边界：活动区 buffer 在下一次用户输入（`turn-begin clearActivity`）时整体清空，故「上一回合的 notice」不会在面板期显示；用户 2026-09-26 裁定**保持现状（仅显示本回合 notice）**，代码不随之改动。
  - **3.1.3 逐项复验通过**（用户 2026-09-26 真机确认）：面板态无光标 / 思考中排队输入光标停在插入点（不跑活动区）/ `Ctrl+L` 后光标仍在输入框 / 退出 TUI 后终端光标可见——四项均通过。

## 收尾

- **文档回写清单**：`docs/DESIGN.md` §布局、`docs/SPEC.md` §11.3、`docs/COMMANDS-SPEC.md` §渲染位置、`docs/IMPLEMENTATION.md`（§排版尺寸唯一来源 + 命令补全键位）、`README.md`（输入区与按键提示区 / 面板）已随实现同步；`docs/BACKLOG.md` 的 3.1.1 / 3.1.2 / 3.1.3 三条标「完成」；本追踪文档移入 `TUI/docs/archived/`。
- **提交记录**：`03594fd`（3.1.3 光标通道）、`76bae69`（3.1.2 提示统一）、`420bd1e`（3.1.1 notice 复用输入区）、`d005343`（3.1.2 补漏：`/model` 面板内键位帮助行）。
- **遗留项（已登记 BACKLOG，交其他 agent）**：3.5.2（demo 冒烟 `titlebar-mode-icons` 期望码位过期——既有缺陷）、3.5.3（根 `npm run test:tui` 包装脚本在本机挂住——既有缺陷）、3.2.7（面板内「自定义回答」编辑无光标，用户裁定先不处理）。
- **范围外未接取**：3.1.4（排版性能）与 3.2 / 3.3 / 3.4 / 3.6 / 3.7 其余条目。
- **行为变化提示（供后续条目参考）**：面板内 `[y]` 红 / `[n]` 绿着色随提示迁移取消（底部提示行为单色）；若要恢复需把 `hintLine` 扩为「段数组」形态，与 3.2.4 / 3.3.1 一并考虑。活动区 buffer 仍按「下一次用户输入清空」的既有语义，面板态 notice 视图因此只覆盖本回合（用户裁定保持现状）。
