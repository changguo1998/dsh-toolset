...
- **会话列表范围（当前目录 / 全部）**：`HistoryPanelState.scope`（`"project"` 默认 / `"all"`，`history-open` 置默认），list 阶段 `Tab` 触发 `history-scope-toggle`。可见列表由纯选择器 `historyVisibleRecords(state)` 收口（project=按 `currentProjectCwd` 精确过滤 cwd；all=全量；**当前目录不可识别 → 空列表**，由渲染层给明确空态，不静默把全量当本目录）。**所有索引读取与收敛都按可见列表**：`history-move` / `history-open-view` / `history-confirm-delete` / `history-delete-done` / `history-clean-done`（`dropHistoryRecords` 收 `visibleIds` 按可见数 clamp）/ `history-refresh`（按可见数 clamp），避免「全量长度」越界或选中不可见项；`index.ts` 的 list Enter / `confirmDeleteRecord` / `openHistoryView` 同样改读 `historyVisibleRecords`。切范围按 id 尽量保留选中项（`history-scope-toggle` 先取旧可见项的 id，在新可见列表里 `findIndex`，找不到则 0）；刷新（`history-refresh`）保留 scope。渲染：`HistoryPanelView` 增加 `records`（可见）/`totalCount`（全量）/`projectCwd`（undefined=识别失败），`layout.ts` 注入；不做第二次 IO（同一份 `listSessions()` 结果过滤）。清理范围**不随列表范围变化**，仍固定当前目录（`cleanableSessionIds` 不变）。

- **会话清理（删除单条 + 清理当前项目空会话）**：list 阶段 `d`/Delete/Backspace 进 `confirm-delete`、`x` 进 `confirm-clean`（`y`/Enter 执行 → `deleting`/`cleaning`，`n`/Esc 取消，进行中吞键），`/session clean` 打开面板直达清理确认。`SessionInfo.isEmpty` 由 adapter 在既有 surface 兜底趟里顺带判定（`persisted && !live && 无 user 消息`；读取失败 → 不判空，避免误删），列表行标 `[空]`。清理范围由纯选择器 `cleanableSessionIds(state)` 收口（同 cwd + persisted + 非 live + 非 current + `isEmpty`；无法确定当前项目 → 空数组，防跨项目误删），**与列表范围解耦**（列表切到「全部」时 `x` 仍只清理当前目录，确认文案明示），`currentProjectCwd` 取活跃会话记录 cwd、回退状态区 cwd。文件级删除 `deleteSessionDir(id, roots)`（`dsh.ts` 导出，语义对齐官方 `deleteSessionLog`）：单段 id 白名单 → 遍历 `sessionRoots()` 首个命中 → `realpath` 包含性校验（会话目录为指向根外的符号链接 → 拒绝）→ `rmSync(recursive, force)`；adapter `deleteSession` 另拒当前活跃会话与内存 live 会话。结果反馈：`history-result` action 把文案写入面板首行（面板占满活动区、notice 归活动区渲染 → 面板打开时 notice 不可见），同时 `notice()` 留痕于对话区；删除/清理成功后重新 `listSessions()` 并以 `history-refresh` 拉回列表（保留高亮位置并 clamp）。
