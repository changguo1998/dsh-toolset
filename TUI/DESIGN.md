...
- **历史区**：左列顶部为**会话标题栏**（2026-09-27 由右侧状态列迁入，置于会话历史区上方）：标题行（前景色；空标题 `<title>` 灰占位保持行稳定）+ 实线下划线（D 列交点用 `┤` 连接，与活动区分隔行中性态一致）；标题优先官方 `dsh-session-title` 服务落盘的 `session/title` 事件折叠结果，缺失时本地兜底为被恢复会话首条用户消息前 30 字符，无消息为 `（新会话）`。标题栏归历史面板（history 焦点时其行左缘框格/分隔竖线随之亮框）；**标题栏行数由对话区承担**（活动区高度不受影响），极矮终端由 `topPaneHeights` 自适应收缩——对话区将不足 1 行时先收掉下划线（标题栏 1 行）、再整栏省略（0 行）以保证对话区非空。正文按 `contentW = historyWidth - FRAME_RIGHT_COLS`（右缘焦点框预留列）换行，沿用 scrollback 语义（wrapping、followBottom、scrollOffset、2000 行上限）。
