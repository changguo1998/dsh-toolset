...
- **历史区/活动区文字右缘留白（`PANE_TEXT_MARGIN_COLS = 1`）**：只有两个 pane 的**文字**排版宽度收窄——横向排列各让 1 列（历史 1 + 活动 1 = 2），纵向排列两 pane 同列同宽、各让 2 列。**所有横线一概不缩**：标题栏下划线、活动区分隔线、回合分隔线（`╌`）、状态栏上下边框均铺满到屏幕最右列（区域外缘框列在横线行补 `─`/`╌` 不留缺口），焦点框矩形也不变。**活动区**另有两处差异：焦点框不画右边框（顶/底亮线直接铺到最右列收尾，无 `┐`/`┘` 角字），内容行行尾不补空格（行到文字右缘为止）。目的是字形宽度算错（CJK / 组合字符宽度估算偏差）时多出的列落在留白里，不顶到外缘框列、不把整行挤到下一行。
