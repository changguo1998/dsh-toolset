...
- **状态区**：以**横向 Box 排版**（`h([环境组, LLM组], {separator})`，组间框线由 Box separator 产出、向上/下横线画交点 `┬`/`┴` 相接成格——上横线竖线向下伸入、下横线竖线向上顶住，组内 `·`），超宽按段折行（折行时组间无框线、无交点）、单组超宽才组内压缩；**最左侧为命令状态符号**（绿 `✓` 成功 / 红 `✗` 失败 / 黄 `○` 运行中 / 黄 `△` 等待交互——审批/问答面板打开 / `?` 不确定状态的回退占位，含初始尚无结果），符号左侧留 1 空格、与时间之间以边框色竖线 `│` 分隔（该竖线亦与上/下横线相接）。数据流见下。
