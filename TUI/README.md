...
1. **免治理**：正常文字/推荐符号/治理区外 → 原样、零反馈；
1. **归一**：命中别名的变体（如 `✔ ✅ → ✓`、`✕ ✖ ✘ ❌ × → ✗`）→ 展示层替换为推荐符号，turn-end 反馈模型纠正；
...
|---|---|---|---|
| 状态 | `✓ ✗ △` | `✔✅→✓`；`✕✖✘❌×🗙→✗`；`⚠→△` | `☑☒`（方框类）、`√`（根号）按几何拆分：不归一；`☑☒` 使用即提醒、`√` 治理区外放行 |
| A 方向箭头 | `→ ← ↑ ↓ ↔`（四向全推荐） | `➔ ➜ ➡ → →` | 若终端把箭头渲染为 2 列（Ambiguous 宽渲染），建议配置 recommended 改用 ASCII（`->` 等） |
| 加减 | ASCII `+ -` | `➕ ➖ → + -` | 优先推荐 ASCII；带圈数字（`❶` 等）不归一不推荐、使用即提醒（用 `1)` 文字序号） |
| 金额 | `¥`（半角） | `￥ → ¥` | 全角 ￥ 归一半角 ¥（治理区外、宽 2 稳定）；`$ € £` 天然放行 |
| 圆域 | `• ◦ ○ ● ◯` | — | 空心点/实心点/空圈/实心圆/大空圈全推荐（均为圆几何、宽 1 稳定；◇ 大空圈与 ○ 同形不另区分） |
| 波浪 | `~`（1 列）/ `〜`（2 列，U+301C） | `～ → 〜` | 全角波浪 ～ 归一到 U+301C；1 列用 ASCII `~` |
| B 三角箭头 | `▶ ◀ ▲ ▼`（四向） | `▸ ► ⏵ ⏩ ➤ → ▶` | 三角箭头四向全推荐（与 ▶ 同风格），其余候选归一 |
| 几何 | `■ □ ◇ ◆` | — | 实心/空心方块与菱形全推荐（宽 1 稳定）；键盘键（`⌘` 等）不推荐（治理区外天然放行） |
| C 双线/推导 | `⟹` | `⇒`（同向右双线） | `⇔`（双向）、`⇐`（左向）按几何拆分：使用即提醒 |
| D 特殊笔触 | （不纳入） | `⇢ ⇝` | 使用即提醒 |
| 列表·圆点 | `•` | — | `◦`（空心圆点）按几何拆分、使用即提醒；三角点（`‣ ⁃` 通用标点区自然放行）、方块（`▪ ▫`）不纳入，方块使用即提醒 |
| 星标 | （不纳入） | | `★☆✦✧` 与 emoji `⭐` 均不推荐；使用即提醒（`⭐` 带颜色明确劝阻） |
| 信息·圈 i | `ⓘ` | `ℹ` | 感叹族推荐 ASCII `!`（`❗❕→!` 归一）；图形族（`💡`）不纳入，使用即提醒 |

判定顺序：零宽跳过 → 别名替换 → 治理区内查推荐（命中放行/未命中记提醒）→ 文字与常用标点放行 → 其余放行。
