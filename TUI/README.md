...
- **渲染相关命令 → 本地小命令表**（app 层直接处理，不经 adapter）：
  ...
  - `/tools [过滤]` — 工具目录**共享列表面板**（活动区）：adapter 经 `ctx.tools.schemas()`（省略 scope = 全局视图）拉取并按 filter 过滤（名称/描述子串）；`↑/↓`、`PgUp/PgDn` 翻页（工具数量通常数十条）、`Enter` 经 `ctx.tools.get(name)` 取详情（名称 + 描述 + 参数 schema）并关面板后以 notice 展示、`Esc` 关闭；宿主未挂载 tools 服务 → 提示不可用且不空开面板
