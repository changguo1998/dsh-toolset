...
- **系统状态区**：横向一行展示时间、当前目录、git 分支与 dirty 标记、模型、上下文长度、缓存命中率（上下文长度与缓存命中率无数据源时为 `—` 占位；模型默认 `—`，`/model` 切换本会话后更新且不落盘）。`StatusTicker` 合并节流读取（5s 一次，一次 tick 批量查 cwd/git/time，避免高频 fork 子进程）。
