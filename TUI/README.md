...
|---|---|---|---|
| 状态 | `✓ ✗ △` | `✔✅☑🗹→✓`；`✕✖✘❌×🗙☒🗷→✗`；`⚠→△` | `☑`/`☒` 与追加符号区 `🗹`/`🗷`（方框勾/叉）为**特例**（2026-11）：带独立方框但并入无框的 ✓/✗，不单独成族；`√`（根号）治理区外放行 |
...
| 几何 | `■ □ ◇ ◆` | `▫◻🔳🔲→□`；`◽▪◼⬛⬜🟥🟦🟧🟨🟩🟪🟫→■`；`🔷🔹🔶🔸⬥⬧→◆`、`⬦⬨→◇` | 实心/空心方块与菱形全推荐（□ U+25A1、■ U+25FC、◇ U+25C7、◆ U+25C6 为代表；空心/实心分族）；emoji 纯色（含 ⬜ 白）→ 实心 ■；双色按钮（🔳🔲 方块按钮，带边框）→ 空心 □（2026-11 订正）；◽（观感实心）→ 实心 ■；文本空心变体（⬦⬨）→ ◇；键盘键（`⌘` 等）不推荐（治理区外天然放行） |
