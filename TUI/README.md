...
配置在 profile 启动时解析，改后需重启 `dsh --profile <p>` 生效。说明：`streamCharsPerSecond` 只在 `streamTypewriter: true` 时生效（mock demo 不经此配置）；`messageGutter` 同时作用于用户块左缘与回复右缘（交错对称；默认 6 → 输入最长折行的左缘与回复正文第 5 个字符同列）；`streamCharsPerSecond` 按码点切分，不会拆断 emoji/CJK。
