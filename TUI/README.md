...
按类分组，**组间以边框色框线 `│` 分隔（与状态栏上/下横线相交相接成格）、组内 `·`**；**最左侧为命令状态符号**（绿 `✓` 成功 / 红 `✗` 失败 / 黄 `●`/`○` 运行中——实心/空心圆按**虚拟速度**交替：真实传输速率经平滑并限幅后驱动，输出越快闪得越快但频率有上下限；无数据时逐渐降频到最低但不停（证明未卡死） / 黄 `△` 等待交互（审批/问答面板打开）/ `?` 不确定状态的回退占位，含初始尚无结果），符号左侧留 1 空格、与时间之间以边框色竖线 `│` 分隔（该竖线亦与上/下横线相接）：
...
- 提示符单字符 = 当前输入模式符号（`>` 普通 / `$` shell / `/` slash，默认前景色）；上一条命令的结果/状态符号在系统状态区最左侧（见上）。
