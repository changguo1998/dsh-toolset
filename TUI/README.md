...
- **渲染相关命令 → 本地小命令表**（app 层直接处理，不经 adapter）：
  ...
  - `/task` — 任务面板（A1，挂 task-engine 只读查询面）：adapter 经 `ctx.taskEngine.query()` 先序展平任务树（嵌套缩进 + 状态 + 待拆分标记）为共享面板行，`Enter` 经 `taskDetail(id)` 查看详情（标题/状态/id/需拆分）后关面板、`Esc` 关闭；宿主未挂载 taskEngine 服务 → 提示不可用且不空开面板
