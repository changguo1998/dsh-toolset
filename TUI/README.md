...
- **顶部区域**：左侧固定 `PLUGIN_WIDTH=2` 列插件窄条（仅竖线 `│`，无边框无标题，当前为空，插件能力后续实现）；右侧**上下分栏**——上部对话区（用户消息 + 模型正文实时流，仅保留最近 3 条模型回复、更早已灰占位 `…(更早回复已折叠)` 折叠，支持滚动 ↑/↓/PageUp/PageDown），下部固定 5 行活动区（流式思考/工具调用/notice 等瞬态结果，`╌` 分隔线隔开、始终底部跟随展示最新；极窄终端自动压缩活动区高度以保证对话区非空）。活动区为**每回合瞬态**：新模型回合开始（turn-begin）即清空上一轮思考/工具/notice，旧命令的活动结果被新回合结果冲掉；顶部/状态/输入三区之间各有横线分隔行（`SEPARATOR_ROWS=2`）。
