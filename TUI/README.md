...
| `TUI/docs/design/NOTICE-LEVELS.md` / `TUI/docs/design/AUDIT-colors.md` | 参考 | notice 级别约定 / 灰度三语义颜色约定 |
...
```
│ 状态列（常驻，最左，Ctrl+S 显隐）    │ 预设名 + 状态符号组 2 空格 会话标题   │
│ Goal active                       │ ─────────────────────────────────   │
│ <objective>                       │ 对话历史（user / assistant 交错）    │
│ ────────────────────────────────  │ ✓ 用户输入块（首行左侧为状态符号）     │
│ Todo 1/3                          │ ...(更早回复已折叠)                 │
│ ○ 待办 / ● 进行中 / ✓ 完成         │ ─────────────────────────────────   │
│ ────────────────────────────────  │ 活动区（思考 / 工具 / notice，瞬态）  │
│ Jobs 运行中/总数                   │ ╌╌ 12:00:00 #3 ╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌╌     │
│                                   │ bash npm run check                 │
────────────────────────────────────┴──────────────────────────────────────
...
- **顶部区域**：最左为常驻状态列（`statusColumnDivisor`，默认 1/3 宽、最低 20 列，历史区保底 10 列；`Ctrl+S` 可整体隐藏——隐藏后状态列宽归 0、右缘分隔竖线不画、历史区吃满整区全宽，显隐随会话写入 `tui-state.json`），其右为区域（会话标题栏 + 对话历史区 + 活动区）。历史区/活动区的**文字**右缘留白只作用于右缘贴外框列的 pane：横向排列时历史 pane 不留白（用户块右缘 `┃` 紧贴内部分隔竖线）、横向活动 pane 与纵向排列的两 pane 各让 1 列，字形宽度算错时多出的列落在留白里、不整行溢出；横线不受影响：标题栏下划线、活动区分隔线、回合分隔线（`╌`）、状态栏上下边框一律铺满到屏幕最右列（含区域外缘框列，不留缺口）；活动区不画右边框，行尾也不补空格。状态列与区域之间为分隔竖线；活动区与历史区之间为实线 `─`，状态列块间与回合之间为虚线 `╌`。
...
标题行（前景色；空标题显示灰色 `<title>` 占位）+ 实线下划线。标题优先取官方 `dsh-session-title` 服务落盘的 `session/title` 事件折叠结果；缺失时本地兜底为被恢复会话首条用户消息前 30 字符，无消息为 `（新会话）`。标题栏行数由对话区承担（不影响活动区高度）。

首行三段：`[preset 图标 + 1 空格 + 预设名] 1 空格 [状态符号组（最多 6 个，空格分隔，顺序固定：沙箱 / policy / plan / verbose / symbol-unify / bell）] 2 空格 [会话标题]`（没有 preset 数据时第一段整段省略，标题左移）。符号是 Nerd Font 私有区字形，各占 1 列（码位见 `layout.ts` 的 `TITLE_ICON`，如沙箱 `U+F03D7`（`md-package_variant_closed`）/ `U+F03D6`（`md-package_variant`）、policy `U+F1739` / `U+F1414`），**颜色即语义值**：沙箱图标（只读 / 可写 / 全权分别绿 / 黄 / 红，其它取值灰）、policy（`ask` 黄 / `never` 绿）、四个开关图标（`on` 绿 / `off` 灰）、preset（拼图图标 + 预设名，默认前景）；`permission` 不再显示（值仍随会话状态快照保存）。窄宽按让位顺序收缩：① 去掉 preset 段 → ② 截断标题 → ③ 去掉整组符号 → ④ 先收掉下划线、再整栏省略，保证对话区非空。**依赖终端字体支持 Nerd Font 私有区字形**（本机验证字体 Maple Mono NF CN），非 Nerd Font 终端会显示豆腐块。

### 对话历史区

- **会话流**：模型正文靠左、右缘留白；用户消息回显为整体靠右的收缩块（块内左对齐、右缘贴历史区右缘、右缘竖线 `┃` 亮红），用户输入与回答之间空一行。左右留白列数默认 6（`messageGutter`）：输入最长折行的左缘与回复正文第 5 个字符同列。
- **用户块状态符号**：每条用户块**首行左侧留白里**渲染 `符号 + 1 空格`（**独立 2 列格**，符号不参与正文换行——正文首行与续行同列、正文列不受符号影响；块右缘位置不变）——终态绿 `✓`（成功）/ 红 `✗`（失败）/ 灰 `■`（turn 被中止）；最新未终态块在忙时黄 `●`/`○`（按虚拟速度交替）、审批/问答面板打开时黄 `△`；其余无终态块（恢复的历史、未收到 turn end 的块）默认前景色 `?`；排队块不显示符号。
...
- **恢复会话按 step 概要恢复工具记录**：resume 时不还原逐条工具行 / 参数摘要 / 结果详情 / thinking——每个**含工具调用**的 step 折成一行 `╌╌ hh:mm:ss #N ╌╌ 工具名[×次数], …[ ✗失败数]`，无工具调用的 step 不出行，整条空文本不产出空行。
...
- **工具行**：`bash <摘要>`（无前缀图标），结果 `✓ <首行>` / 失败红色 `✗ <详情>`；每步以 `╌╌ hh:mm:ss #N ╌╌╌…` 整行虚线分隔（本地时区 24 小时制、逐段补零；事件缺时间时只出 `#N`），连续两次调用之间不空行。
- **空白分片**：宿主每步末尾常补发纯空白文本块（`"\n\n"`），在「上一行是异 kind 或缓冲为空」时被丢弃，不再在活动区 / 历史区留下成片空行；同一 kind 内部的段落空行（分隔段落）保留，丢弃后下一条流式分片另起一行、不与上一行粘连。
- **详略两态**：`/verbose off` 紧凑模式把每条目压成 1 行 + 行尾 `…`，便于高密度浏览长任务输出。
- **面板优先**：审批 / 问答 / 模型选择 / 各列表面板打开时占满活动区可视行，瞬态行本帧让位。

### 状态列

自上而下 **Goal / Todo / Jobs 三块**，块间以虚线 `╌` 分隔（原 Mode 块已整块迁入标题栏符号组，见上；`Ctrl+S` 整体显隐状态列——隐藏后该列宽归 0、右缘分隔竖线不画、历史区吃满整区全宽，显隐随会话写入 `tui-state.json`）：
...
agent 工作中提交的消息经官方流程立即交给核心（`followup`，`next-turn` 队列，逐条不合并），本机只登记「已交给核心、本回合尚未认领」的文本用于显示：钉在历史 pane 可视窗口右下角，右对齐用户块 + **灰色**右缘竖线 `┃`（区别于已发出消息的亮红竖线、且不显示状态符号），不随历史滚动；核心开始新回合认领该条时，它转正为历史里的用户消息。历史视口相应收缩若干行。`Esc` 与 `Alt+Enter` 会把登记内容按提交顺序退回输入框。**压缩上下文期间（`compaction/start` → `compaction/end`）该会话按活跃处理**：用户块符号显示运行中、新消息同样走排队、`Ctrl+D` 退出守卫不触发（`Esc` 中断语义不变）。

### 系统状态区

按类分组，**组内与组间分隔统一为 `•`**（U+2022；默认前景色、1 列、两侧无空格）。状态符号不再由状态栏承载（改渲染在每条用户块首行左侧，见上「对话历史区」），组间也没有边框色竖线 `│`，因此状态栏上/下横线不再有组间交点 `┬`（状态列右缘 D 列的 `┴` 仍保留）：
...
- 提示符单字符 = 当前输入模式符号（`>` 普通 / `$` shell / `/` slash，默认前景色）；上一条用户输入的结果 / 状态符号不在状态栏，而在**该用户块首行左侧留白里**（见上「对话历史区」）。
...
- 按键提示区（1 行，与输入区之间不画横线，**任何状态都占 1 行**）：文案唯一来源是 `src/app/layout/hints.ts` 的 `hintLine(state)`，按当前状态切换——普通输入 `[Alt+Enter]打断并发送 · [Ctrl+L]重绘 · [Ctrl+J]输入换行 · [Ctrl+S]状态列 · [/help]更多命令`；命令补全为补全键位；历史会话按阶段；审批 / 问答 / 模型选择 / 状态选项 / `/jobs` / 各列表族面板显示该面板自己的键位（面板内不再内嵌提示）。空文案（如历史加载中）显示空行，高度不变。
...
| `Ctrl+L` | 强制整帧重绘 |
| `Ctrl+S` | 切换垂直状态列显隐（隐藏后状态列宽归 0、分隔竖线不画、历史区吃满整区全宽；显隐随会话写入 `tui-state.json`） |
