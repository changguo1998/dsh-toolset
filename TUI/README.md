...
- **顶部区域**：左侧固定 `PLUGIN_WIDTH=2` 列插件窄条（仅竖线 `│`，无边框无标题，当前为空，插件能力后续实现）；右侧新起一列「**顶部状态列**」（约 25% 宽窄列，显示当前会话 goal 详细目标/阶段/阻塞原因 + todo 列表；goal 目标最多 5 行、每条 todo 最多 3 行，超限折叠为 `…(+N行)`；无 goal/todo 显示灰色占位「（无目标/待办）」；**PgUp/PgDn 滚动状态列**）再往右为**对话历史区**（用户消息 + 模型正文实时流，仅保留最近 3 条模型回复、更早已灰占位 `…(更早回复已折叠)` 折叠，支持滚动 ↑/↓），历史区下部为固定 5 行活动区（流式思考/工具调用/notice 等瞬态结果，`╌` 分隔线隔开、始终底部跟随展示最新；极窄终端自动压缩活动区高度以保证对话区非空）。活动区为**每回合瞬态**：新模型回合开始（turn-begin）即清空上一轮思考/工具/notice，旧命令的活动结果被新回合结果冲掉；顶部/状态/输入三区之间各有横线分隔行（`SEPARATOR_ROWS=2`）。
...
- **渲染相关命令 → 本地小命令表**（app 层直接处理，不经 adapter）：
  ...
  - `/goal` — 当前会话目标迷你面板（goal 状态徽标 + todo 计数只读展示；↑/↓ 滚动、Esc 关闭）。目标/待办**详细内容常驻顶部状态列**（左半窄列，PgUp/PgDn 滚动）；/goal 面板为全宽大视图
