...
首行三段：`[preset 图标 + 1 空格 + 预设名] 1 空格 [状态符号组（最多 6 个，空格分隔，顺序固定：沙箱 / policy / plan / verbose / symbol-unify / bell）] 2 空格 [会话标题]`（没有 preset 数据时第一段整段省略，标题左移）。符号是 Nerd Font 私有区字形，各占 1 列（码位见 `layout.ts` 的 `TITLE_ICON`，如沙箱 `U+ED75` / `U+ED95`、policy `U+F1739` / `U+F1414`），**颜色即语义值**：沙箱图标（只读 / 可写 / 全权分别绿 / 黄 / 红，其它取值灰）、policy（`ask` 黄 / `never` 绿）、四个开关图标（`on` 默认前景 / `off` 灰）、preset（拼图图标 + 预设名，默认前景）；`permission` 不再显示（值仍随会话状态快照保存）。窄宽按让位顺序收缩：① 去掉 preset 段 → ② 截断标题 → ③ 去掉整组符号 → ④ 先收掉下划线、再整栏省略，保证对话区非空。**依赖终端字体支持 Nerd Font 私有区字形**（本机验证字体 Maple Mono NF CN），非 Nerd Font 终端会显示豆腐块。

### 对话历史区

- **会话流**：模型正文靠左、右缘留白；用户消息回显为整体靠右的收缩块（块内左对齐、右缘贴历史区右缘、右缘竖线 `┃` 亮红），用户输入与回答之间空一行。左右留白列数默认 6（`messageGutter`）：输入最长折行的左缘与回复正文第 5 个字符同列。
- **用户块状态符号**：每条用户块**首行左侧**渲染 `符号 + 1 空格 + 文本`（在块内部，块右缘位置不变，故块宽比纯文本时多 2 列）——终态绿 `✓`（成功）/ 红 `✗`（失败）/ 灰 `■`（turn 被中止）；最新未终态块在忙时黄 `●`/`○`（按虚拟速度交替）、审批/问答面板打开时黄 `△`；其余无终态块（恢复的历史、未收到 turn end 的块）默认前景色 `?`；排队块不显示符号。
...
- 提示符单字符 = 当前输入模式符号（`>` 普通 / `$` shell / `/` slash，默认前景色）；上一条用户输入的结果 / 状态符号不在状态栏，而在**该用户块首行左侧**（见上「对话历史区」）。
