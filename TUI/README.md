...
- **渲染相关命令 → 本地小命令表**（app 层直接处理，不经 adapter）：
  ...
  - `/quit` — 关闭 renderer 退出
  - `/session` — 会话面板：列出持久化会话（newest-first，live 会话标记 `[当前]` 不可续），Enter 切换到选中的 persisted 会话（先释放当前 agent，再经 host `agents.resume` 恢复继续对话；resume 失败进面板 error 态不崩溃）；`d`/Delete 删除选中会话（二次确认）、`x` 清理当前项目空会话（二次确认）、`/session clean` 直达清理确认（文件级删除：安全 id + realpath 包含性校验，范围仅当前项目）
