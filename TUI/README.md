...
- **系统状态区**：按可用宽度展示时间、**会话标题**、当前目录、git 分支与 dirty 标记、模型、上下文长度、缓存命中率（标题段紧随时间，>24 列终端显示；resume 后取自被恢复会话首条用户消息前 30 字符，无消息为 `（新会话）`）；内容过宽时溢出到多行并压缩顶部区域（上下文长度与缓存命中率无数据源时为 `—` 占位；模型默认 `—`，`/model` 切换本会话后更新且不落盘）。`StatusTicker` 合并节流读取（5s 一次，一次 tick 批量查 cwd/git/time，避免高频 fork 子进程）。
...
- **会话切换**：`/session` 面板列出持久化会话，Enter 即切换（单活跃会话：释放旧 agent → `agents.resume` 恢复目标会话 → 其历史表面与标题载入 buffer/状态栏，之后可继续对话）；live 会话标记 `[当前]` 不可续；切换后发新消息落入被恢复会话的记录。
