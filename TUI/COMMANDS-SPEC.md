...
**§3 排除项的源码核实**：`/clear` —— **B1 复核（2026-09，dsh-session 0.1.5-rc.2）**：store 公开面仍无会话删除/清理 API（`SessionStore.detachEntered`/`store.delete` 为 private 生命周期 teardown，非对外命令面；`dsh-session-query` 0.1.5-rc.2 亦仅查询；无 `session/delete` 事件）→ 宿主入口仍缺；且「会话清理」用户诉求已由 TUI 自建的文件级删除覆盖（`/session` 面板 `d` 删单条 / `x` 清理空会话 / `/session clean`，见 README「会话清理」段）——`/clear` 若语义为清上下文则更与宿主上下文管理无关，维持排除；`/login` `/logout` —— **B2 复核（2026-09，dsh-credentials 0.1.5-rc.2）**：`ctx.credentials` 为**凭据存取 seam**——reference 空间 `resolve`/`describe`/`set`/`unset`（index.d.ts:129/136/145/152）+ record 空间 `listRecords`/`describeRecord`/`modifyRecord`/`deleteRecord`（index.d.ts:174/165/186/191）+ `credentials/reference-updated` 事件；README 无交互登录/auth 关键词、无 login/signin/oauth/device 方法 → 仅凭据**引用/记录管理**原语，**无交互式登录流程**（供应商选择 / OAuth / 设备码）；`/logout` 亦无宿主「登出」概念（`unset`/`deleteRecord` 是删凭据的运维操作，非登出）；由 TUI 自造属凭据录入与安全边界设计，超出命令范围；`/review` —— **B3 复核（2026-09，dsh-workflow 0.1.5-rc.2）**：`workflowEngine.start(WorkflowStartRequest)` 存在（`dsh-workflow/lib/types/index.d.ts:119`）但为**通用脚本执行引擎**——调用方须自备 `script`（plain-JS 体）+ `meta`（`WorkflowMeta` 身份块：name/description/whenToUse?/phases?，`types.d.ts:39-48` 引擎仅 shape 校验）+ `parent: Agent`（`runtime-types.d.ts:15-29`）；`$PKG/lib` 无任何 review 资产、无内置命名工作流/preset；故 `/review` 需**先建 review 编排资产**（并行 reviewer 人数/验收口径/相位编排）并授权当前活跃 Agent 为 parent——属编排层资产设计，超出命令接线范围。均按能力／语义缺口排除，**非**「API 未找到」。
...
| `/workflows` ✅ 已实现（P2#16） | 需插件改造 | dsh-workflow 无只读查询面 → 走事件面：adapter 在 `tool-workflow/*` 桥接处按 runId 维护运行集合（不 emit 增量，防事件流污染），`refreshWorkflows` 读集合推面板；挂载探测 `ctx.workflowEngine`；TUI 面板打开定时刷新（C2 同款） |
| `/council [N]` ✅ 已实现（P2#18） | 需插件改造 | 核实 `ctx.subagents.start`（dsh-subagent 0.1.5-rc.2：SubagentStartRequest=label/prompt/parent/signal/agentOptions?，SubagentRun 不 reject 于子级失败）可达 → adapter 并行拉起 N 个评审子代理 allSettled 汇总（stopReason=error 降级）；App 目标源=goal 快照 objective 或最近用户输入；notice 展示 |
| `/search <query>` ✅ 已实现（P2#24） | 需插件改造 | 核实 `ctx.web`（dsh-web WebRuntime index.d.ts:80 `search(request, signal)`）+ provider 注册 seam → adapter 经 `web.search` 归一化行（title ?? host、payload=url）推 listPanel；多引擎聚合语义由 host seam 承载，TUI 侧不重复实现 |
