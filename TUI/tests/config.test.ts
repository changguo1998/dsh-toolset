...
test("normalizeConfig：合法整数保留，非法/越界回落缺省（undefined）", () => {
  ...
  const bad = normalizeConfig({ layout: { activityHeightDivisor: "x", statusColumnDivisor: NaN } })!;
...
test("metricsFor：footerHeight 绝对行数生效；statusDivisor 控制状态列宽（历史区保底 10 列）", () => {
  ...
  assert.equal(cfg.statusColWidth, Math.min(30, 50), "statusDivisor=2 → cols/2=30");
});
