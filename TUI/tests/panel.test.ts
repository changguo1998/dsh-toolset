...
import type { Paragraph, Box } from "../src/app/layout/box.ts";
...
/** 以 fill 摊平节点 → 纯文本行 */
function rowsOf(node: Box | Paragraph, w = 40): string[] {
...
test("panelTitle：返回 Paragraph（bold）且 fill 渲染为标题全文", () => {
  const t = panelTitle("等待审批");
  assert.equal(t.kind, "text");
  assert.deepEqual(t.style, { bold: true });
  assert.deepEqual(rowsOf(t), ["等待审批"]);
});

test("panelTitle：自定义样式与 bold 合并", () => {
  const t = panelTitle("标题", { fg: "blue" });
  assert.deepEqual(t.style, { bold: true, fg: "blue" });
});

test("panelQuestion/panelExplanation：Paragraph + 透传样式", () => {
  const q = panelQuestion("问题？");
  assert.equal(q.kind, "text");
  assert.equal(q.style, undefined, "无样式时 style 省略（对齐 fill 语义）");
  const e = panelExplanation("说明", { fg: "gray" });
  assert.deepEqual(e.style, { fg: "gray" });
...
test("panelOptions：单选标记（焦点>黄 / 选中*绿 / 普通）", () => {
  ...
  // 焦点行 prefix 持黄样式、选中行持绿样式
  const f = box.children[0] as Paragraph;
  assert.deepEqual(f.prefix?.style, { fg: "yellow" });
  const s = box.children[1] as Paragraph;
  assert.deepEqual(s.prefix?.style, { fg: "green" });
});
...
test("panelOptions：多选标记 + 与游标关闭", () => {
  ...
  const n = rowsOf(
    panelOptions([{ label: "c", selected: true }], { cursor: "", multiSelect: true }),
  );
...
test("panelOptions：自定义焦点/选中样式透传", () => {
  ...
  const f = box.children[0] as Paragraph;
  assert.deepEqual(f.prefix?.style, { fg: "red" });
});
