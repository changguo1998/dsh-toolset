...
export const DEFAULT_ALIASES: Readonly<Record<string, string>> = {
  "✔": "✓",
  "✅": "✓", // 方框勾 emoji（绿底方框）→ 细线 ✓
  "☑": "✓", // 方框勾（U+2611）特例：带独立方框、并入未加框的勾
  "✕": "✗",
  "✖": "✗",
  "✘": "✗",
  "❌": "✗",
  "🗙": "✗",
  "☒": "✗", // 方框叉（U+2612）特例：并入未加框的叉
  "🗷": "✗", // 方框叉（U+1F5F7，追加符号区 emoji）特例：并入未加框的叉（同 ☒）
  "🗸": "✓", // 浅勾（U+1F5F8）→ 细线 ✓
  "🗹": "✓", // 方框勾（U+1F5F9，追加符号区 emoji）特例：并入未加框的勾（同 ☑）
  "⚠": "△", // 警告三角：emoji 呈现为填色，与 ✓/✗ 细线风格不一致 → 换空心三角 △
  // 箭头家族 A（线条）：各方向与其同向的其它箭头归一到该向推荐（参考右向）
  ...
  "⬆": "↑",
  "⬇": "↓",
  // 箭头家族 B（三角）：各方向与其同向的其它三角归一到该向代表（参考右向 ▶ U+25B6）
  "▸": "▶",
  "►": "▶",
  "⏵": "▶",
  "⏩": "▶",
  ...
  "⏬": "▼", // 黑双下三角（U+23EC，同 ⏪→◀）
  // 箭头家族 C（双线/推导）：各方向双线归一到该向长双线代表（参考右向
  ...
  "～": "〜",
  // 圆族 emoji 归一（emoji 按「设计含色数」归类——单色填充一律实心，
  // 只有含内空腔/双色的算空心，如 ⭕ 圆环）：
  ...
  // 方块族（空心代表 □ U+25A1 / 实心代表 ■ U+25FC）：尺寸/填充变体归一到代表
  //   emoji 中纯色 → ■（含 ⬜ 白大方、⬛ 黑大方、各色大方）；🔳🔲（方块按钮=带边框双色）→ □
  "▫": "□",
  "◻": "□",
  "🔳": "□", // 白底+深色边框（双色设计）→ 空心
  "🔲": "□", // 黑方块按钮（黑底+浅色边框，双色设计）→ 空心（同 🔳）
  "◽": "■", // 观感/渲染为实心 → 实心
  "⬜": "■", // 纯白大方（单色）→ 实心
  "▪": "■",
  "◼": "■",
  ...
  "🟪": "■",
  "🟫": "■",
  // 菱形族（◆ 实心 / ◇ 空心 为代表）：菱形 emoji + 文本尺寸变体归一到对应推荐符
  //   emoji 纯色（🔶🔸 橙、🔷🔹 蓝）→ ◆；文本空心变体 ⬦⬨ → ◇
  "🔷": "◆", // 大蓝实心菱形
  "🔹": "◆", // 小蓝实心菱形
  "🔶": "◆", // 大橙菱形（纯色 → 实心）
  ...
  "⬦": "◇", // 中等空心菱形（U+2B26，文本变体）
  "⬨": "◇", // 中小空心菱形（U+2B28）
  // 三角族（▲▼ 为代表）：红色三角 emoji 归一到细线 ▲▼
  "🔺": "▲",
  "🔼": "▲",
  "🔻": "▼",
  "🔽": "▼",
  // 空心三角族：空心为独立一族（与实心 ▶◀▲▼ 不互相归一），
 ...
 * - 双线族（C）按方向归一：⇒→⟹、⇐→⟸、⇔→⟺（短双线 → 同向长双线代表）
 * - 空心三角族：▷◁△▽ 四向代表，族内变体（▹▻◃◅▵▿）归一到该向代表；
 *   空心/实心（▲▼◀▶）各成一族、不互相归一
 * - ◦（25E6 空心圆点）：空心/实心各成一族，◦ 为空心圆点族代表、已入默认白名单 → 放行
 * 特例：☑/☒（方框勾/方框叉）与 🗹/🗷（追加符号区方框勾/叉）虽带独立方框，
 *   不各自成族，而是并入对应的细线符号——✅☑🗹→✓、❌☒🗷→✗（「特殊情况，对应没有框的勾和叉」）。
 * 原则：空心/实心 = 不同形状身份，各成一族、不互相归一；
 *   只归一修饰性差异（emoji 上色、粗细、大小、重复数量、内缀）。
 * emoji 形状按「设计含色数」归类：单色填充一律实心（含白/黑）；
 *   只有含内空腔或双色设计的算空心：
 *   圆族 ⚪⚫🔴…🟤→●、⬤（U+2B24 黑大圆，文本几何同 ● 仅大小）→●、⭕（圆环）→○；方块族 ▫◻🔳🔲→□、◽/⬜/▪◼⬛/各色大方→■；
 *   菱形族 🔶🔸🔷🔹⬥⬧→◆、⬦⬨（文本空心）→◇；三角族 🔺🔼→▲、🔻🔽→▼；
 *   空心三角族（文本几何 ▷◁△▽）四向代表入白名单、族内变体归一到代表。
 ...
 * 处理。勾/叉/警示/圆/方块等带色或 emoji 呈现的符号归入，细线变体不归入。
 */
