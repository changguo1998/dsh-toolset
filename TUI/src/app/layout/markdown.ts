...
/** 零宽字符区间表（对齐 Markus Kuhn wcwidth 零宽表，紧凑分组）。
 *  模块级常量：原实现把它声明在 isZeroWidthChar 内，每次调用都重建并线性扫描
 *  326 条区间——逐字符宽度计算的主要常数因子。
...
/** 零宽字符判定（模块级区间表，见 ZERO_WIDTH_RANGES） */
...
export function charWidth(ch: string): number {
  ...
  const remembered = layoutCacheEnabled() ? CHAR_WIDTH_TABLE[cp]! : 0;
  if (remembered !== 0) return remembered - 1;
  const width = computeCharWidth(cp);
  if (layoutCacheEnabled()) CHAR_WIDTH_TABLE[cp] = width + 1;
  return width;
}
...
export function displayWidth(text: string): number {
  return memo(displayWidthCache, text, () => {
    let w = 0;
 ...
 */
const wrapInlineCache = createTextCache<FrameSegment[][]>();
...
const wrapCodeCache = createTextCache<FrameSegment[][]>();

export function wrapCodeLine(text: string, width: number): FrameSegment[][] {
  return memo(wrapCodeCache, sizedKey(text, width), () =>
    codeLineRows(text, width),
  );
}

/** wrapCodeLine 直算路径（无缓存）：整行主题灰底并补齐到内容区宽度 */
function codeLineRows(text: string, width: number): FrameSegment[][] {
  if (text === "") return [[]];
 ...
 */
const wrapAssistantCache = createTextCache<FrameSegment[][]>();
