...
/**
 * 后台任务面板 Box 生成器（DESIGN.md §7 / SPEC.md §7）：标题 + 运行中
...
export function buildJobsPanelBox(
  ...
  const headSegs: { text: string; style?: import("../../renderer/screen.ts").FrameStyle }[] = [
  ...
  // 空列表占位
  if (jobs.length === 0) {
    leaves.push(styled([seg("（无后台任务）", { fg: "gray" as ColorName })], { wrap: false }));
