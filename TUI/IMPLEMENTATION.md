...
- `/search <query>` — 网页搜索：`adapter.search(query, maxResults=10)`——经 `ctx.web.search({query, maxResults})`（`dsh-web` WebRuntime 统一搜索 seam，`dsh-web-search-deepseek` 等 provider 经 `registerSearchProvider` 注册后由 seam 聚合——多引擎语义承载在宿主，TUI 侧仅聚合展示）归一化行（title 缺省回落 `new URL(url).host`、detail=url·snippet、payload=url）推 `command-panel-data(kind=search)`；`content` 摘要随事件带出；宿主未挂载 web.search → reject（App 前置检查 warn 不空开面板）；复用共享列表面板（Enter 展示来源 URL、翻页同款）。
