...
| `mode {sessionId,kind,value}` | `modeBySession[sid]` 三合一 | 状态列 Mode 块：各项以竖线 ` | ` 连续排布、放不下折行（折行处不加竖线），属性名默认前景、未生效值灰、生效项强调色，Mode↔Goal 虚线分隔（2026-09-07 由状态栏迁入；见 DESIGN 渲染语义）（B2） |
