...
- **非打印控制字符清洗（渲染保护，2026-09-28）**：流式文本进 buffer 前经 `sanitizeText`（state.ts）清洗——CRLF/孤立 CR 归一为换行（否则 `\r` 残留被终端当回车、抹掉整行内容造成大段空白），其余 C0/C1 控制字符（含 Tab、孤立 ESC）剔除，完整 ANSI 转义序列（CSI `ESC[…`、OSC `ESC]…BEL/ST`）保留（渲染着色功能，/copy 时再剥离）。剔除计数累计入 `state.strippedChars`（turn-begin 清零），turn-end 后 App 侧 `warnStrippedChars()` 若有剔除则以黄色 notice 提示「已过滤 N 个非打印控制字符」。恢复历史（`surfaceToBuffer`）同样走 `sanitizeText` 且按 `\n` 拆成独立 buffer 行，避免行内嵌换行破坏帧布局。
...
| `goal-change {sessionId,operation,goal\|cleared,…}` | `goalBySession[sid]` 快照替换/clear 墓碑 | 顶部状态列详显（B1，按 sessionId 隔离；状态栏 `goal:<phase>` 徽标已于 2026-09-17 移除，由右侧状态列承接；同日 goal 块与 todo 块之间加点更少的虚线 `╌` 分隔；2026-09-07 `/goal` 面板移除、仅提示查看信息栏，状态列补 `Todo 完成数/总数` 标题与 `○`/`●`/`✓` 标记（待办空心圆/进行中实心圆、对号不划线、进行中全黄、续行缩进对齐、jobs 完成项正文灰+删除线）、新增 jobs 块、状态区上方分隔改单线 `─`；对话 turn 之间改点更少的虚线 `╌`（窗口间分隔统一实线：活动区分隔/状态区上下 2026-09-07 均 `─`，状态列块间仍 `╌`）；2026-09-07 默认焦点改 null：新输入/输出（内容推进 action：append/thinking/notice/tool/turn/compaction/jobs 等）后自动回无焦点，Tab 才进入循环；状态列折叠改等级递进「未溢出完整显示，溢出按 L0 全显/L1 隐藏已完成/L2 仅进行中(goal 压标题)/L3 进行中 1 行 从低到高尝试」） |
...
- **服务接线**：`adapter/dsh.ts` 经 `runtime.on("user-questions/request", answerer)` 注册 waterfall 应答者（dsh-user-questions 契约：返回 `QuestionAnswer` 即占用请求、`next()` 放行；与 `approval/request` 同构）——单活动请求守卫（并发请求直接 reject）、`signal` abort → reject、无 UI 监听者/已释放时 `next()` 委托；DSH 回调提问归一化为 `DshEvent {type:'question'; id; questions[]}`。App 应答走 `answerQuestion(id, {answers})`（整批），Esc/cancel 走 `cancelQuestion(id)`（仅 reject）。`emit_models` 等既有逻辑不受影响。
...
- 输入态底部新增独立的**按键提示区**（1 行，`layout.ts` 导出 `HINT_LINE`）：`[Enter]发送 · [Alt+Enter]打断并发送 · [Esc]打断 · [Ctrl+L]重绘 · [/help]更多命令`，正常前景色，窄终端按显示宽度截断；**与输入区之间不画横线**；审批/问答/模型选择面板自带操作提示，不显示该区。
